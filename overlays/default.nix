{inputs}: [
  ...
  (final: prev: {
    shadps4 = prev.shadps4.overrideAttrs (finalAttrs: prevAttrs: {
      version = "0.16.0-unstable-2026-06-05";

      src = prevAttrs.src.override {
        tag = null;
        rev = "5b8b66ab124532146f16f2014ce06d858cfd03c8";
        hash = "sha256-OV7fRvqqYAhQGl4Ewn2KADUnYsN6RT2GYZkJKpHgL1k=";
  ...
  (final: prev: {
    dusklight-rando = prev.dusklight.overrideAttrs (finalAttrs: prevAttrs: {
      ...
      src = prevAttrs.src.override {
        ...
        postCheckout = ''
          ...
          if git apply --check ${../patches/dusklight/aurora-restore-fifo-drains.patch} 2>/dev/null; then
            echo "dusklight-rando: aurora-restore-fifo-drains.patch applies — drop the patches filter in overlays/default.nix" >&2
            exit 1
          fi
        '';
      };
      ...
      patches =
        builtins.filter
        (p: !final.lib.hasSuffix "aurora-restore-fifo-drains.patch" (baseNameOf (toString p)))
        prevAttrs.patches;

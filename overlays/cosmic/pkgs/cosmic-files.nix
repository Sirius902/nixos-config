final: prev:
prev.cosmic-files.overrideAttrs (finalAttrs: prevAttrs: {
  version = "1.0.0-alpha.7-unstable-2025-08-14";

  src = prevAttrs.src.override {
    tag = null;
    rev = "bf81364e0be7aacc0ea4763766202ea052b889da";
    hash = "sha256-HafohSVCMcaM6p/41HSieB/BdnNhMB7sYAWwZ7RIkpQ=";
  };

  cargoHash = "sha256-TDXo0PsDLIBewAasBK82VsG1O0lPqY6g3dBRFsGzF6A=";

  cargoDeps = final.rustPlatform.fetchCargoVendor {
    inherit (finalAttrs) pname src version;
    hash = finalAttrs.cargoHash;
  };

  # FUTURE(Sirius902) One of the tests is broken currently due to a missing arg. Re-enable once it is fixed.
  # https://github.com/pop-os/cosmic-files/blob/bf81364e0be7aacc0ea4763766202ea052b889da/src/tab.rs#L6470
  doCheck = false;

  passthru.updateScript = final.nix-update-script {
    extraArgs = [
      "--version-regex"
      "epoch-(.*)"
    ];
  };
})
